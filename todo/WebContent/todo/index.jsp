...
<%
	...
	ArrayList<UserTodoVO> userToDoList = dao.getAllToDoListById(LoginedUser);	
%>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
				...
				<form action="todoForm.jsp" method="post">
					<div class="panel panel-info">
						...
						<%
							...
							for (UserTodoVO Usertodo : userToDoList) {
						%>

						<tbody>
							<tr>
								<td><a href="detail.jsp?no=<%=Usertodo.getTodo().getNo()%>"><%=Usertodo.getTodo().getNo() %></a></td>
								<td><a href="detail.jsp?no=<%=Usertodo.getTodo().getNo()%>"><%=Usertodo.getTodo().getDay()%></a></td>
								<td><a href="detail.jsp?no=<%=Usertodo.getTodo().getNo()%>"><%=Usertodo.getTodo().getCompleted()%></a></td>
								<%
									if (Usertodo.getTodo().getCompleted().equals("yes")) {
								%>
								<td>
 ...
 %>
								...
								<td><a href="completeProcess.jsp?no=<%=Usertodo.getTodo().getNo()%>"
									class="btn btn-success btn-sm">완료</a> <%
 	}
 %> <a href="deleteProcess.jsp?no=<%=Usertodo.getTodo().getNo()%>"
